...
instruments ([Perrier et al. 2003](https://ui.adsabs.harvard.edu/abs/2003A&A...410.1039P/abstract), [Butler et al. 2006](https://ui.adsabs.harvard.edu/abs/2006ApJ...646..505B/abstract), [Wittenmyer et al. 2009](https://ui.adsabs.harvard.edu/abs/2009ApJS..182...97W/abstract)). In
...
Note the clear offset between ELODIE data (in red above) and the other
...
model1 = RVmodel(fix=False, npmax=1, data=HD106252_combined)
```
...
model2 = multi_instruments(run=False)
```
...
```python
_, axs = res.plot_posterior_PKE(show_prior=True)
axs
```


    
![png](multi_instrument_files/multi_instrument_29_0.png)
...
These issues can be easily solved by setting slightly more informative priors for the RV offsets and some of the orbital parameters. We can use a couple of helper functions available in `pykima.utils` to assign appropriate Gaussian priors.


```python
from kima.pykima.utils import (
    get_gaussian_prior_vsys, 
    get_gaussian_priors_individual_offsets
)
```

Set an informative prior for the systemic velocity:


```python
Cprior = get_gaussian_prior_vsys(HD106252_combined, use_std=True)
...
and for each of the RV offsets:


```python
i_offset_prior = get_gaussian_priors_individual_offsets(
    HD106252_combined, use_std=True
)

print(i_offset_prior)
```

    [Gaussian(-15590.4; 59.0117), Gaussian(-15583.9; 61.2813), Gaussian(-15518.5; 102.095)]
    

as well as a narrower prior for the semi-amplitude


```python
Kprior = kima.distributions.Uniform(0, 500)
```
...
```python
model.Cprior = Cprior
model.individual_offset_prior = i_offset_prior
model.conditional.Kprior = Kprior

kima.run(model, steps=1000, progress_bar=True)
```

    # Seeding random number generators. First seed = 1781773617.
...
```
    ...
    Loading files took 0.04 seconds
    log(Z) = -633.14
    Information = 21.52 nats
    BMD = 0.00
    Effective sample size = 1.0
...
```python
res1.hist_vsys(show_prior=True)[0]
```


    
![png](multi_instrument_files/multi_instrument_48_0.png)
...
```python
_, axs1 = res1.plot_posterior_PKE(show_prior=True)
axs1
```


    
![png](multi_instrument_files/multi_instrument_50_0.png)
...
```python
p = res1.maximum_likelihood_sample()
res1.phase_plot(p)
```

    Sample with the highest likelihood value  (logL = -611.62)
    -> might not be representative of the full posterior distribution
    
    jitter:
      [  0.          35.33119501   3.85178015  56.72670339 133.27933597]
    number of planets:  1
    orbital parameters:           P           K          M0           e           w 
                         1611.06336   161.34740     2.76678     0.57857     4.38881
    instrument offsets:  (relative to ELODIE) 
                         HET                   HJS                   Lick                 
                         -15611.868            -15616.238            -15552.939           
    vsys:  15552.84315839
    


    
![png](multi_instrument_files/multi_instrument_52_1.png)
